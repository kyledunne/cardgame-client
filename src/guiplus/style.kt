...
fun toTexture(texturePath: String): Texture {
    return TextureLoader.getTexture(texturePath.substring(texturePath.length - 3), ResourceLoader.getResourceAsStream(texturePath))
}

class Image(texturePath: String, var backgroundColor: Color? = null, var bindingColor: Color? = null): Style {
...
class Animation(var textures: Array<Texture>, var ticksPerFrame: Int, var backgroundColor: Color? = null, var bindingColor: Color? = null): Style {
    var numFrames = textures.size
    var currentFrame = 0
    var currentTick = 0

    fun update() {
        if (currentTick == ticksPerFrame - 1) {
            currentTick = 0
    ...
    override fun draw(rect: Rect) {
        var texture = textures[currentFrame]
        backgroundColor?.draw(rect)
        val textureWidth = texture.width
        val textureHeight = texture.height
        GL11.glEnable(GL11.GL_TEXTURE_2D)
